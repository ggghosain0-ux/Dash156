...
async function initDatabase() {
    ...
    try {
        ...
        const defaultSettings = [
            { key: 'site_name', value: 'VOIDORA CLOUD DASHBOARD', description: 'Site name/title' },
            { key: 'site_logo', value: '', description: 'Site logo filename' },
            { key: 'site_favicon', value: '', description: 'Site favicon filename' },
            { key: 'site_description', value: 'VOIDORA CLOUD - Best Reliable Cloud & VPS Hosting Solutions', description: 'Site meta description' },
            { key: 'contact_email', value: process.env.SMTP_FROM || 'support@voidoracloud.com', description: 'Contact email address' },
...
function getRazorpayInstance(settings = {}) {
    ...
    const key_secret = settings.razorpay_key_secret || process.env.RAZORPAY_KEY_SECRET || 'AbCdEfGhIjKlMnOpQrSt';
...
app.use(async (req, res, next) => {
    res.locals.siteName = 'VOIDORA CLOUD DASHBOARD';
    res.locals.siteLogo = '/images/default-logo.png'; 
    res.locals.siteFavicon = '/images/favicon.ico'; // Default favicon
    res.locals.siteDescription = 'VOIDORA CLOUD - Best Reliable Cloud & VPS Hosting Solutions';
...
app.post('/create-razorpay-order', requireAuth, async (req, res) => {
    ...
    try {
        ...
        const client = getRazorpayInstance(res.locals.settings);
        const order = await client.orders.create(options);
        // Update order with Razorpay order ID
        await runDb('UPDATE orders SET razorpayOrderId = ? WHERE id = ?', [order.id, orderId]);
        res.json({
            orderId: order.id,
            key: res.locals.settings?.razorpay_key_id || process.env.RAZORPAY_KEY_ID || 'rzp_test_1234567890abcdef',
...
app.post('/retry-payment/:orderId', requireAuth, async (req, res) => {
    ...
    try {
        ...
        const client = getRazorpayInstance(res.locals.settings);
        const rzOrder = await client.orders.create(options);
